...
HelpLibraryManagerLauncher.exe /product "VS" /version "100" /locale en-us /uninstall /silent /vendor "Kay McCormick" /productName "WPF Application Infrastructure" /mediaBookList "WpfApp" > NUL
...
REM build time.
IF EXIST "WpfApp.msha" COPY /Y "WpfApp.msha" HelpContentSetup.msha
...
REM Uninstall first in case it is already there.  If not, it won't install below.  We'll ignore any error output
REM by redirecting it to NUL.
HelpLibraryManagerLauncher.exe /viewerVersion %1  /locale en-us /wait 0 /operation uninstall /vendor "Kay McCormick" /productName "WPF Application Infrastructure" /bookList "WpfApp" > NUL

REM Install the new content.
HelpLibraryManagerLauncher.exe /viewerVersion %1  /locale en-us /wait 0 /operation install /sourceUri "%CD%\WpfApp.msha"
